...
def bfs(pacman) -> list:
    ...
    if not single_pacman:
        goals = order_goals(pacman.goal_state, pacman.init_state)
    else:
        goals = [pacman.goal_state[0]]
    all_food = []

    while pacman.goal_test_func(all_food)==False and goals:
        ...
        while not frontier.empty():
            ...
            if pacman.node == curr_goal:
                ...
                if single_pacman:
                    ...
                    pacman.node = curr_goal


                    all_food.append(pacman.node)
            ...
            if single_pacman:
                successors = pacman.successor_func(pacman.node)
            else:
                successors = pacman.successor_func(pacman.node, curr_goal)
