...
class SingleFoodSearchProblem:
    ...
    def successor_func(self):
        ...
        if self.goal_state[0] >= row and self.goal_state[1] <= column:
            successors.sort(key = cmp_to_key(sortbyCond_ar_dc))
        elif self.goal_state[0] >= row and self.goal_state[1] >= column:
    ...
    def goal_test_func(self):
        return self.node == self.goal_state
    ...
    def animate(self) -> None:
        if self.solution[0]!='Stop':
            ...
            for i in self.solution:
                ...
                if i == 'N':
                    self.state[self.init_state[0]-1] = update(self.state[self.init_state[0]-1], self.init_state[1], 'P')
...
class MultiFoodSearchProblem:
    def __init__(self, maze_file):
        ...
        self.init_state, self.goal_states = self.ini()

        # Khởi tạo vị trí ban đầu cho node (pacman)
        self.node = self.init_state

        # Vị trí được cập nhật trong mỗi hành động của  (Lúc đầu chưa cập nhật nên là None)
    ...
    def goal_test_func(self):
        ...
        #             return False
        #     return True
        # return ch(self.goal_state, all_food)
